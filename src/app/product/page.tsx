  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const fetchedProducts: Product[] = data.data || []; // Fallback in case data is undefined
        setFilteredProducts(fetchedProducts);

        // Safely handle categories and colors
        const uniqueCategories = [...new Set(fetchedProducts.map((product) => product.category))];
        setCategories(uniqueCategories as string[]);

        const uniqueColors = [...new Set(fetchedProducts.flatMap((product) => product.colors))];
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {paginatedData.map((product) => (
          <div
            key={`${product.productName}-${product.price}-${product.category}`} // Ensure unique key
